...
function parseXls(data: Buffer): Array<Record<string, unknown>> {
  ...
  // Nanotax report exports prepend 2 metadata rows (e.g. "Printed: ..." and report title)
  // before the real column headers. Find the row that contains "Client code" and use
  // that as the header row, regardless of how many leading metadata rows there are.
  const headerRowIdx = raw.findIndex(row => (row as unknown[]).some(cell => cell === "Client code"));
